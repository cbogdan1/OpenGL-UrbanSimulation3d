		...
		for (size_t s = 0; s < shapes.size(); s++) {
			...
			for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
				...
				for (size_t v = 0; v < fv; v++) {
					...
					if (vx < minBound.x) minBound.x = vx;
					if (vy < minBound.y) minBound.y = vy;
					if (vz < minBound.z) minBound.z = vz;

					if (vx > maxBound.x) maxBound.x = vx;
					if (vy > maxBound.y) maxBound.y = vy;
					if (vz > maxBound.z) maxBound.z = vz;
